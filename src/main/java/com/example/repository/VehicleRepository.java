...
public interface VehicleRepository extends JpaRepository<Vehicle, Long> {
    ...
    @Query(value = "SELECT v.* FROM vehicles v "
            ...
            + "AND (:brandName IS NULL OR unaccent(b.name) ILIKE unaccent('%' || CAST(:brandName AS VARCHAR) || '%')) "
            + "AND (:manufactureYear IS NULL OR v.manufacture_year = :manufactureYear) "
            + "AND (:minPrice IS NULL OR v.price >= :minPrice) "
            + "AND (:maxPrice IS NULL OR v.price <= :maxPrice) "
            + "AND (:ownerId IS NULL OR o.id = :ownerId) "
            + "AND (:ownerName IS NULL OR unaccent(o.name) ILIKE unaccent('%' || CAST(:ownerName AS VARCHAR) || '%'))",
            nativeQuery = true)
    ...
    @Query(value = """
        ...
        JOIN brands b ON b.id = v.brand_id 
        WHERE (v.price > 10000000 AND (
            unaccent(b.name) LIKE unaccent('S%') 
        ))
